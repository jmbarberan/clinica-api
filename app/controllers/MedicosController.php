...
class MedicosController extends ControllerBase  {
  ...
  public function pacientesBuscarAction() {
    ...
    $this->response->setContent(json_encode($res,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
    $this->response->send();
  }
  ...
  public function pacientePorCedulaAction() {
    ...
    $this->response->setContentType('application/json', 'UTF-8');
    $this->response->setContent(json_encode($ret,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
    $this->response->send();
  }
  ...
  public function consultasUnificarPacienteAction() {
    ...
    $this->response->setContentType('application/json', 'UTF-8');
    $this->response->setContent(json_encode($msj,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
    $this->response->send();
  }
  ...
  public function claveGuardarAction() {
    try {
      ...
      if ($datos->id > 0) {
        // Traer servicio por id
        $cve = MdTablasRegistros::findFirstById($datos->id);
        $cve->tabla_id = $datos->tabla_id;
        $cve->indice = $datos->indice; 
        $cve->denominacion = $datos->denominacion; 
        $cve->codigo = $datos->codigo;
        $cve->contenedor = $datos->contenedor; 
        $cve->valor = $datos->valor;
        $cve->estado = $datos->estado;
        if($cve->update()) {
          $ret->res = true;
          $ret->cid = $datos->id;
          $ret->ind = $datos->indice;
          $ret->msj = "Se actualizo correctamente los datos";
          $this->response->setStatusCode(200, 'Ok');
        } else {
          $msj = "No se puede actualizar los datos: " . "\n";
          foreach ($cve->getMessages() as $m) {
            $msj .= $m . "\n";
          }
          $ret->res = false;
          $ret->cid = $datos->id;
        ...
        // traer indice nuevo
        $cod = MdTablasRegistros::maximum([
          'column' => 'indice',
          'conditions' => 'tabla_id = ' . $datos->tabla_id
        ]) ?? 0;
        $indice = intval($cod);
        $indice += 1;
        // Crear registro nuevo        
        $cve = new MdTablasRegistros();
        $cve->tabla_id = $datos->tabla_id;
        $cve->indice = $indice; 
        $cve->denominacion = $datos->denominacion; 
        $cve->codigo = $datos->codigo;
        ...
        if ($cve->create()) {
          ...
          $ret->cid = $cve->id;
          $ret->ind = $indice;
          $ret->msj = "Se creo correctamente el nuevo registro";
          $this->response->setStatusCode(201, 'Created');  
        } else {
